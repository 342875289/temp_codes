...
u8 TX_PLOAD_WIDTH = 24;   //发射数据通道有效数据宽度0~32Byte 
u8 RX_PLOAD_WIDTH = 24;
u8 RF_DR_PWR = 0x0f;//发送/接收数据的速率与功率
u8 TX_ADDRESS[TX_ADR_WIDTH] = {0x34,0x43,0x01};  // 定义一个静态发送地址
u8 RX_ADDRESS[RX_ADR_WIDTH] = {0x34,0x43,0x00};
...
void NRF_ID_init(enum NRF_ID id,u8 id_number,enum NRF_RATE datarate)
{
	if(id == master)//NRF主机设置
	{
		TX_ADDRESS[TX_ADR_WIDTH-1]=0x01;//发送目的地址默认设为1号从机
		RX_ADDRESS[RX_ADR_WIDTH-1]=0x00;//主机接收地址默认为0x00
		RF_DR_PWR = datarate;
	}
	else if(id == slave)//NRF丛机设置
	{
		TX_ADDRESS[TX_ADR_WIDTH-1]=0x00;//发送目的地址设为主机
		RX_ADDRESS[RX_ADR_WIDTH-1]=id_number;//接收地址设为自身编号
		RF_DR_PWR = datarate;
	}
}

void Change_TX_Target_Address_By_modbusCmd(void)
{
	TX_ADDRESS[TX_ADR_WIDTH-1] = NRF_TX_BUF_Point[2];
}
...
void NRF_RX_Mode(void)
{
	 ...
	 SPI_NRF_WriteReg(NRF_WRITE_REG+SETUP_AW,0x01);    //使用3Byte传输地址
   ...
   SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,RF_DR_PWR); //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
void NRF_TX_Mode_add_address(void)
{  
	NRF_CE_LOW();		
...
u8 NRF_Rx_Dat(u8 *rxbuf)
{
	...
	while(NRF_Read_IRQ()!=0)
	{
		if(newCmd == 1  || newSelfCmd == 1 )return ChangeToUSART;
	}
