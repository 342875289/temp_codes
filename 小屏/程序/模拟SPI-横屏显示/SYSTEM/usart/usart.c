...
void getInformationToOLED(u8 USART_BUFF[])
{
	...
	if(( USART_BUFF[0] == 0x77 )  )
	{
		if( USART_BUFF[1] == 0x0D )
		{
			...
			x_angle =  x_i + x_f/10000.0;
			y_angle =  y_i + y_f/10000.0;
			z_angle =  z_i + z_f/10000.0;
			
			dispalyAngleByDegree(x_angle,string);
			LCD_ShowString(offset + 115 + 16,120,string);
			dispalyAngleByDegree(y_angle,string);
			LCD_ShowString(offset + 115 + 16,155,string);
			dispalyAngleByDegree(z_angle,string);
			...
			if( z_angle>=25.0)
			{
				...
				if(isalert == 0)
				{
					showhanzi(offset + 220,125,11);  //警告
					showhanzi(offset + 220,160,12); 
					isalert = 1;
				}
				LCD_ShowString(offset + 115 + 16,190,string);
				POINT_COLOR=BLACK;
			}
			else
			{
				if(isalert == 1)
				{
					showhanzi(offset + 220,125,13); //
					showhanzi(offset + 220,160,13); 
					isalert = 0;
				}
				LCD_ShowString(offset + 115 + 16,190,string);
			}
		...
		else if( USART_BUFF[1] == 0x05 && USART_BUFF[3] == 0x1F)
		{
...
void dispalyAngleByDegree(float angle,unsigned char string[])
{
	...
	if(angle>=0){ string[0] = ' '; }
	else        { string[0] = '-'; angle = - angle;}
	
	angle_i = (int)angle;
	angle_f = (int)((angle - angle_i)*1000);
	...
	if(string[1] == '0' && string[2] == '0'){string[1] = ' ';string[2] = ' ';}
	else if(string[1] == '0'){string[1] = ' ';}
}
